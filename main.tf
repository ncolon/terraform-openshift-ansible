...
${var.storage_count > 0 ? "[glusterfs]" : ""}
${var.storage_count > 0 ? "${join("\n", formatlist("%v glusterfs_devices='[ ${local.gluster_storage_devices} ]' openshift_node_group_name='node-config-compute'", var.storage_hostname))}" : ""}
...
data "template_file" "openshift_inventory" {
    template = <<EOF
